...
class Azure:
    ...
    def get_secrets(self, ROOT_PATH):
        with open('AZURE_SECRET_CREDENTIALS.json') as f:
    ...
    def train(self):
        ...
        self.codeError = -1
        try:
            CF.large_person_group.train(large_person_group_id=self.largePersonGroupId)
            print("TRAINED!!!!")
        except requests.exceptions.ConnectionError:
            self.codeError = self.INTERNET_ERROR
        except:
    ...
    def request_add_face(self, imageBytes, largePersonGroupId, personId, targetFace):
        ...
        headers, data, json = {'Content-Type': 'application/octet-stream'}, imageBytes, None
    ...
    def add_face(self, imageBytes, person_id, frame):
        self.codeError = -1
        # print('add_face')
    ...
    def get_all_names(self):
        ...
        try:
            ...
            for person in persons:
                groupInfo.append({'name':person['name'], 'personId':person['personId']})
    ...
    def extract_attributes(self, responseDetection):
        ...
        bald = r['hair']['bald']
        if r['hair']['hairColor']:
            hairColor = r['hair']['hairColor'][0]['color']  # asumo que el 0 es el mayor
        else:
            hairColor = ""
    ...
    def delete_person(self, personId):
        try:
            CF.large_person_group_person.delete(self.largePersonGroupId, person_id=personId)
    ...
    def delete_person_without_face(self):
        try:
            responseInfo = CF.large_person_group_person.list(self.largePersonGroupId)
            print(responseInfo)
